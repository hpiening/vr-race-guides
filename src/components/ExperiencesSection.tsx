...
export default function ExperiencesSection({ data, basePath = 'sections.experiences', theme = 'classic' }: Props) {
  ...
  if (theme === 'trailhead') return <ExperiencesTrailhead data={data} basePath={basePath} editing={editing} />
  ...
  return (
    <section id="experiences" className="bg-vr-offwhite px-6 md:px-12 py-20 md:py-[104px]">
      <div className="max-w-[1180px] mx-auto">
        ...
        <div className="grid md:grid-cols-[1.3fr_1fr] border border-vr-line rounded-lg overflow-hidden bg-vr-white mb-9">
          <div className="p-9 flex flex-col justify-center">
            <span className="font-micro uppercase text-vr-sky block mb-2 text-[11px] tracking-[0.14em]">
              Official lodging partner · <EditableText as="span" value={data.lodging.partner} path={`${basePath}.lodging.partner`} />
            </span>
            <h3 className="font-heading uppercase text-vr-forest mb-3 text-[24px] tracking-[0.02em]">Book your weekend stay</h3>
            <p className="font-body text-vr-forest/85 leading-[1.6] max-w-[460px] mb-5 text-[15px]">
          ...
          <div className="grid gap-6 md:grid-cols-2 mb-4">
            {data.activities.map((act, i) => (
              <div key={i} className="border border-vr-line bg-vr-white rounded-lg p-7 flex flex-col">
                <div className="flex items-start gap-2">
                  <h3 className="font-heading uppercase text-vr-forest mb-3 flex-1 text-[18px] tracking-[0.02em]"><EditableText as="span" value={act.name} path={`${basePath}.activities.${i}.name`} /></h3>
                  <ListControls path={`${basePath}.activities`} index={i} count={data.activities.length} />
                </div>
                <p className="font-body text-vr-forest/85 leading-[1.6] mb-4 flex-1 text-[15px]"><EditableText as="span" value={act.description} path={`${basePath}.activities.${i}.description`} /></p>
                {(act.discountCode || editing) && (
                  <p className="font-micro text-sm mb-3">
                    Use code{' '}
                    <span className="font-label tracking-widest bg-vr-forest text-vr-cream px-2 py-0.5 rounded text-xs inline-block"><EditableText as="span" value={act.discountCode ?? ''} path={`${basePath}.activities.${i}.discountCode`} placeholder="CODE" /></span>
                  </p>
                )}
                {editing ? (
                  <EditableUrl path={`${basePath}.activities.${i}.url`} label="Link URL" />
                ) : (
        ...
        <div className="mb-12"><AddButton path={`${basePath}.activities`} item={{ name: 'New activity', description: '', url: '' }} label="Add activity" /></div>
          ...
          <>
            ...
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 mb-4">
              {data.hikes.map((hike, i) => {
                    ...
                    <div className="p-6">
                      {editing ? (
                        <div className="space-y-1 mb-2">
                          <EditableText as="div" className="font-micro uppercase text-vr-sky text-[10px] tracking-[0.12em]" value={hike.distance} path={`${hp}.distance`} placeholder="3.5 mi" />
                          <EditableText as="div" className="font-micro uppercase text-vr-sky text-[10px] tracking-[0.12em]" value={hike.difficulty} path={`${hp}.difficulty`} placeholder="Moderate" />
                        </div>
                      ) : (
                        <div className="font-micro uppercase text-vr-sky mb-1.5 text-[10px] tracking-[0.12em]">
                          {[hike.distance, hike.difficulty].filter(Boolean).join(' · ')}
                        </div>
                      )}
                      <h4 className="font-heading uppercase text-vr-forest mb-1.5 text-[17px] tracking-[0.02em]"><EditableText as="span" value={hike.name} path={`${hp}.name`} /></h4>
                      <p className="font-body text-vr-forest/70 text-[13px]"><EditableText as="span" value={hike.elevation} path={`${hp}.elevation`} /></p>
                      {editing && (
                        <div className="mt-2">
                ...
                return editing ? (
                  <div key={i} className="border border-vr-line bg-vr-white rounded-lg overflow-hidden relative">
                    <div className="absolute top-2 right-2 z-10"><ListControls path={`${basePath}.hikes`} index={i} count={data.hikes.length} /></div>
                    {inner}
                  </div>
                ) : (
                  <a key={i} href={hike.url} target="_blank" rel="noopener noreferrer" className="border border-vr-line bg-vr-white rounded-lg overflow-hidden transition-shadow hover:shadow-[0_14px_30px_rgba(38,69,51,0.14)] block">{inner}</a>
                )
              })}
            </div>
            <div className="mb-11"><AddButton path={`${basePath}.hikes`} item={{ name: 'New hike', distance: '', elevation: '', difficulty: '', url: '' }} label="Add hike" /></div>
          </>
        )}
        ...
        {((data.sights && data.sights.length > 0) || editing) && (
          <>
            <h3 className={subhead}>Iconic Views &amp; Sights</h3>
            <div className="grid gap-4 mb-4" style={{ gridTemplateColumns: 'repeat(auto-fill,minmax(210px,1fr))' }}>
              {(data.sights ?? []).map((s, i) => {
                const sp = `${basePath}.sights.${i}`
                const body = (
                  <>
                    <div className="flex items-start gap-2">
                      <h4 className="font-heading uppercase text-vr-forest mb-1.5 flex-1 text-[16px] tracking-[0.02em]"><EditableText as="span" value={s.name} path={`${sp}.name`} /></h4>
                      {editing && <ListControls path={`${basePath}.sights`} index={i} count={data.sights!.length} />}
                    </div>
                    <p className="font-body text-vr-forest/85 leading-[1.55] text-[14px]"><EditableText as="span" value={s.description} path={`${sp}.description`} /></p>
                    {editing && <EditableUrl path={`${sp}.url`} label="URL (optional)" />}
                  </>
                )
                return !editing && s.url ? (
            ...
            <div className="mb-11"><AddButton path={`${basePath}.sights`} item={{ name: 'New sight', description: '' }} label="Add sight" /></div>
          </>
        )}

        {/* Know before you go */}
        {(data.parkNote || editing) && (
          <div className="mt-9 bg-vr-deep rounded-lg px-7 py-6 flex flex-wrap gap-3.5 items-center">
            <span className="font-heading uppercase text-vr-sky shrink-0 text-[14px] tracking-[0.04em]">Know before you go</span>
            <p className="font-body text-vr-cream/85 leading-[1.55] flex-1 min-w-[260px] text-[14px]"><EditableText as="span" value={data.parkNote ?? ''} path={`${basePath}.parkNote`} /></p>
          </div>
        )}
        ...
        {(data.restaurants.length > 0 || editing) && (
          <>
            ...
            <p className="font-body text-vr-forest/85 leading-[1.6] max-w-[620px] mb-8 text-[17px]"><EditableText as="span" value={data.restaurantsIntro || 'A few of the popular local food and drink spots in and around Estes Park.'} path={`${basePath}.restaurantsIntro`} /></p>
                ...
                return (
                  <div key={i} className="border border-vr-line bg-vr-white rounded-lg p-6">
                    <div className="flex items-center gap-2.5 mb-2">
                      {r.icon && <span className="text-[20px]">{r.icon}</span>}
                      <h4 className="font-heading uppercase text-vr-forest flex-1 text-[17px] tracking-[0.02em]"><EditableText as="span" value={r.name} path={`${rp}.name`} /></h4>
                      <ListControls path={`${basePath}.restaurants`} index={i} count={data.restaurants.length} />
                    </div>
                    {!editing && r.address && <div className="font-micro text-vr-forest/60 mb-1 text-[12px]">{r.address}</div>}
                    {!editing && (r.phone || r.url) && (
                      <div className="flex gap-3.5 mb-3 font-micro text-[12px]">
                    ...
                    <p className="font-body text-vr-forest/85 leading-[1.55] text-[14px]"><EditableText as="span" value={r.description} path={`${rp}.description`} /></p>
                    {editing && (
                      <div className="mt-2 space-y-0.5">
                        <EditableText as="div" className="font-micro text-xs text-vr-mid" value={r.icon ?? ''} path={`${rp}.icon`} placeholder="Emoji (optional)" />
                        <EditableText as="div" className="font-micro text-xs text-vr-mid" value={r.address ?? ''} path={`${rp}.address`} placeholder="Address (optional)" />
                        <EditableText as="div" className="font-micro text-xs text-vr-mid" value={r.phone ?? ''} path={`${rp}.phone`} placeholder="Phone (optional)" />
                        <EditableUrl path={`${rp}.url`} label="Website URL" />
                      </div>
                    )}
                  </div>
                )
